...
process_execute(const char *command) {
    ...
    check_executable = palloc_get_page(0);
    if (check_executable == NULL)
        return TID_ERROR;
    strlcpy(check_executable, cmd_copy, PGSIZE);


    char *exec_file = strtok_r(check_executable, " ", &save_ptr);
    if (!filesys_open(exec_file)) {
        palloc_free_page(cmd_copy);
        palloc_free_page(check_executable);
        return TID_ERROR;
    }
