...
static bool is_thread(struct thread *)UNUSED;
                                      static void *alloc_frame(struct thread *,
                                                               size_t size);
                                      ...
                                      void thread_recalculate_recent_cpu_all_threads(
                                              void);
                                      void thread_yield_for_higher_priority(
                                              void);
...
thread_get_by_tid(int tid) {
    ...
    for (it = list_begin(&all_list);
         ...
         it = list_next(it)) {
        struct thread *elth = list_entry(it,
...
check_sleeping_threads(void) {
    ...
    while (it != list_end(&sleeping_list)) {
        ...
        struct thread *th = list_entry(it,
        struct thread, elem);
...
thread_yield_for_higher_priority(void) {
    ...
    if (!list_empty(&ready_list)) {
        struct list_elem *max = list_max(&ready_list, thread_cmp_priority,
                                         NULL);
        struct thread *th_max = list_entry(max,
        struct thread, elem);
...
thread_recalculate_recent_cpu(struct thread *th, void *aux UNUSED) {
    ...
    th->recent_cpu = FPR_MUL_FPR(part3,
                                 th->recent_cpu);    // (...) * recent_cpu
...
thread_recalculate_priority(struct thread *th, void *aux UNUSED) {
    th->priority = PRI_MAX - FPR_TO_INT(FPR_DIV_INT(th->recent_cpu, 4)) -
                   (th->nice * 2);
}
...
thread_set_nice(int new_nice) {
    ...
    struct list_elem *elem_ready_thread_max = list_max(&ready_list,
                                                       thread_cmp_priority,
                                                       NULL);
    struct thread *ready_thread_max = list_entry(elem_ready_thread_max,
    struct thread, elem);
...
bool thread_cmp_priority(const struct list_elem *a,
    ...
    struct thread *th_a = list_entry(a,
    struct thread, elem);
    struct thread *th_b = list_entry(b,
    struct thread, elem);
...
next_thread_to_run(void) {
    if (list_empty(&ready_list))
        ...
        struct list_elem *el_th_max = list_max(&ready_list,
                                               thread_cmp_priority, NULL);
        struct thread *th_max = list_entry(el_th_max,
        struct thread, elem);
